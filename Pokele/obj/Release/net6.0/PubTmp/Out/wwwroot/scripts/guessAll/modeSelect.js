...
const getChosenGeneration = async () => {
  ...
  try {
      const response = await fetch(`http://noobtriestowin-001-site1.ctempurl.com/api/User/${userId}/GetChosenGeneration`, {
      method: 'GET',
      headers: {
  ...
  modes.forEach(mode => {
    mode.addEventListener("click", async (event) => {
        ...
        if (isLoggedIn && mode.id != "continue") {
            ...
            fetch(`http://noobtriestowin-001-site1.ctempurl.com/api/User/${userId}/SetChosenGeneration`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
...
async function handleMode(mode) {
  ...
  const getRecordTime = async (userId, generation) => {
    try {
        const response = await fetch(`http://noobtriestowin-001-site1.ctempurl.com/api/User/${userId}/GetRecordTime?generation=${generation}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
...
const resetGuessedPokemon = async () => {
    ...
    try {
        const response = await fetch(`http://noobtriestowin-001-site1.ctempurl.com/api/User/${userId}/ResetGuessedPokemon`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        }
