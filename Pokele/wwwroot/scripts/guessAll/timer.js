...
async function initializeTimer() {
  try {
    const userId = sessionStorage.getItem('userId');
      const response = await fetch(`${apiUrl}/User/${userId}/GetTimeSoFar`, {
      method: 'GET',
      headers: {
...
function stopTimer() {
  ...
  let gen = document.querySelectorAll(".genGuess");
  if(gen.length > 1){
    gen = "all"
  }
  else{
  ...
  const userId = sessionStorage.getItem('userId');
    ...
    fetch(`${apiUrl}/User/${userId}/ChangeRecordTime`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
...
const setTimeSoFar = async (userId, time) => {
  try {
      const response = await fetch(`${apiUrl}/User/${userId}/SetTimeSoFar`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
